...
public class Main {
    public static void main(String[] args) {
        ...
        personelListesi.forEach(p -> System.out.println(p.getAd() + " - " + p.getYaş()));
        ...
        long buyuk30 = personelListesi.stream()
                .filter(p -> p.getYaş() > 30)
                .count();

        System.out.println("Yaşı 30'dan büyük olan personellerin sayısı: " + buyuk30);
        
        long aHarfiIleBaslayanPersonelSayisi = personelListesi.stream()
                .filter(p -> p.getAd().startsWith("A"))
                .count();
                
        System.out.println("A harfi ile başlayan personel sayısı: " + aHarfiIleBaslayanPersonelSayisi);
        
        Personel enBuyukYasliPersonel = personelListesi.stream()
                .max(Comparator.comparing(Personel::getYaş))
                .orElse(null);
        ...
        Optional<Personel> ali = personelListesi.stream()
                .filter(p -> p.getAd().equals("Ali"))
                .findFirst();

        if (ali.isPresent()) {
            System.out.println("Ali'nin yaşı: " + ali.get().getYaş());
        } else {
            System.out.println("Personel listesinde Ali yok!");
        }
        
        List<String> enKucukUc = personelListesi.stream()
                .sorted(Comparator.comparingInt(Personel::getYaş))
                .limit(3)
                .map(Personel::getAd)
        ...
        long yas2535 = personelListesi.stream()
                .filter(p -> p.getYaş() >= 25 && p.getYaş() <= 35)
                .count();
        System.out.println("Yaşı 25 ile 35 arasında olan personellerin sayısı: " + yas2535);
        
        personelListesi.stream()
        .filter(p -> p.getAd().length() > 4)
        .forEach(p -> System.out.println(p.getAd()));
        
        personelListesi.stream()
        .sorted(Comparator.comparingInt(Personel::getYaş).reversed())
        .limit(2)
        .forEach(p -> System.out.println(p.getAd()));
        
        personelListesi.stream()
        .filter(p -> p.getAd().endsWith("a"))
        .map(p -> p.getAd().toUpperCase())
        .forEach(System.out::println);
        
        double ortalama = personelListesi.stream()
                .mapToInt(p -> p.getAd().length())
                .average()
                .orElse(0.0);

		System.out.println("İsimlerin uzunluğunun ortalaması: " + ortalama);
		
		int yas = personelListesi.stream()
		...
		boolean varMi = personelListesi.stream()
			    .anyMatch(p -> p.getAd().toLowerCase().contains("ahmet"));
	        ...
	        List<String> tersSiraliIsimler = isimler.stream()
	                .map(s -> new StringBuilder(s).reverse().toString())
	                .sorted()
	                .collect(Collectors.toList());

	        System.out.println("Ters çevrilmiş isimlerin alfabetik sıralaması: ");
	        tersSiraliIsimler.forEach(System.out::println);		
